...
module.exports = {
    ...
    register: async (req, res, next) => {
        try {
            ...
            const isExist = await UsersService.isExist(req.body);

            if (isExist) {
                return commonResponse.CustomError(res, "EMAIL_ALREADY_EXISTS", 404);
            }

            if (req.files?.image) {
                req.body.image = fileupload.uploadFile("registeration", req.files.image);

                s3Client.uploadImage(`${STORAGE_PATH}/${req.body.image}`, req.files.image.name);
            }

            req.body.password = await commonFunctions.encryptStringCrypt(req.body.password);

            req.body.otp = await otpGenerator.generate(6, { upperCaseAlphabets: false, specialChars: false, digits: true, lowerCaseAlphabets: false });

            req.body.otp_exoiration_time = new Date(moment().add(5, "minutes"));
            ...
            if (user) {
                ...
                const templatePath = path.join(__dirname, "../..", "views", "send-otp.ejs");

                let emailData = {};

                ejs.renderFile(templatePath, htmlData, function (err, data) {
                    if (err) {
                        console.log("🚀 ~ file: users.controller.js:52 ~ err:", err);
                    } else {
                        emailData = {
                            to: user.email,
                ...
                commonResponse.success(res, "USER_CREATED", 201, getUser);
            } else {
